...
def img2label_paths(img_paths):
    return [path.replace("images", "annotations").replace("visible", "").replace("jpg", "txt").replace("png", "txt")
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # default is visible
        path = os.path.join(path, "visible")
        try:
            f = []  # image files
            for p in path if isinstance(path, list) else [path]:
                p = Path(p)  # os-agnostic
                if p.is_dir():  # 如果是目录，直接访问目录下图片
                    f += glob.glob(str(p / '**' / '*.*'), recursive=True)
                elif p.is_file():  # 如果是文件，读取文件中图片路径
                    with open(p, 'r') as t:
            ...
            self.lwir_files = sorted([str(x).replace("visible", "lwir") for x in self.img_files])
        ...
        self.lwir_files = [str(x).replace("visible", "lwir") for x in self.img_files]
    ...
    def __getitem__(self, index):
        ...
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
        lwir = lwir[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        lwir = np.ascontiguousarray(lwir)
        # data = data[..., ::-1]  # BGR->RGB
        # data = cv2.resize(data, (128, 128)).transpose(2, 0, 1)
        img_aware = img_aware[..., ::-1].transpose(2, 0, 1)
        img_aware = np.ascontiguousarray(img_aware)

        return torch.from_numpy(img), torch.from_numpy(lwir), torch.from_numpy(img_aware), labels_out, self.img_files[
            index], shapes

    @staticmethod
    def collate_fn(batch):
        img, lwir, img_aware, label, path, shapes = zip(*batch)  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.stack(lwir, 0), torch.stack(img_aware, 0),torch.cat(label, 0), path, shapes
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        # img, lwir, (h0, w0), img.shape[:2]
        img, lwir, (h0, w0), (h, w) = load_image(self, index)

        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            img5 = np.full((s * 2, s * 2, lwir.shape[2]), 114, dtype=np.uint8)
        ...
        img5[y1a:y2a, x1a:x2a] = lwir[y1b:y2b, x1b:x2b]
        padw = x1a - x1b
        padh = y1a - y1b
    ...
    img5, labels4 = random_perspective(img5, labels4,
                                       degrees=self.hyp['degrees'],
                                       translate=self.hyp['translate'],
                                       scale=self.hyp['scale'],
                                       shear=self.hyp['shear'],
