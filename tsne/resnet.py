...
RES_model = 'resnet101'  # model type
...
class ResNetExt(object):

    def __init__(self, model):
        self.model = model

    def extract(self):
        img_dir = "/home/shw/data/UVA/val/images/visible"
        res_model = ResidualNet(model=self.model)
        res_model.eval()
        ...
        for cls_name in os.listdir(img_dir):
            # print(cls_name + "..." + str(len(features)))
            cls_id = 0
            for name in os.listdir(os.path.join(img_dir, cls_name)):
                img_path = os.path.join(img_dir, cls_name, name)
                # img = scipy.misc.imread(img_path, mode="RGB")
                # img = scipy.misc.imresize(img, size=(16, 16))
        ...
        plt.ylabel('number')
        plt.hist(val, 255,stacked=True,alpha=0.5)
        plt.show()
        # feature_mat = features
        # np.savetxt("features.txt", feature_mat)
        # label_mat = np.array(labels)
        # np.savetxt("labels.txt", label_mat)
...
if __name__ == "__main__":
    ...
    # img_dir = "G:\graduation\dataset\streetDataset\hk-kmeans-data"
    # print(os.listdir(img_dir))

    ext = ResNetExt("resnet18")
    ext.extract()
