...
def extract_weights(template, weights, destination, mask_list):
    ...
    for k, v in model_items:
        ...
        if layer_type in ['Focus']:
            ...
            idx_cv3_in = np.squeeze(
                np.argwhere(np.asarray(torch.cat([mask_out[1][-1], mask_out[2]], dim=0).cpu().numpy())))
            ...
            indx_in_list = [np.squeeze(np.argwhere(np.asarray(mask_in.cpu().numpy()))) for mask_in in
                            mask_list[layer_idx]]
            if 'm.0.weight' in k:
    ...
    for i, n_ch in enumerate(n_ch_list):
        if i < len_backbone - 1:
            ...
            if layer_type == 'Focus':
                ...
                assert len(args) == 5
                args = [n_ch_list[i][-1], True, 1, [round(e, 10) for e in e_list[i]],
                        [round(e, 10) for e in ein_list[i]]]  # default: c2, shortcut=True, g=1, e=0.5
                model_dict['backbone'][i][3] = args
            elif layer_type == 'SPP':
                assert len(args) == 3
            ...
            if layer_type == 'Conv':
                ...
                assert len(args) == 5
                args = [n_ch_list[i][-1], False, 1, [round(e, 10) for e in e_list[i]],
                        [round(e, 10) for e in ein_list[i]]]  # default: c2, shortcut=True, g=1, e=0.5
                if i == len_backbone - 1:
                    model_dict['backbone'][i][3] = args
                else:
    ...
    for i, m in enumerate(model.model):
        if i < len_backbone - 1:
            ...
            if isinstance(m, Focus):
                mask = cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=m.conv.conv,
                                                  block_bn=m.conv.bn)
                ...
                mask_cv2 = cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=m.cv2.conv,
                                                      block_bn=m.cv2.bn)
                mask_cv3 = cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=m.cv3.conv,
                                                      block_bn=m.cv3.bn)
                ...
                mask_list1 = [cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=block_t[t].cv1.conv,
                                                         block_bn=block_t[t].cv1.bn) for t in range(len_bottleneck)]
                mask_list2 = []

                m_cv1_conv_weight, m_cv1_bn_weight = m.cv1.conv.weight.data.abs().clone(), m.cv1.bn.weight.data.abs().clone()
                weight_alpha_list2 = [torch.mean(
                    block_t[t].cv2.conv.weight.data.abs().clone().view(block_t[t].cv2.conv.weight.data.size()[0], -1),
                    dim=1)
                                      for t in range(len_bottleneck)]
                weight_copy_list2 = [10 * torch.mean(m_cv1_conv_weight.view(m_cv1_conv_weight.size()[0], -1),
                                                     dim=1) * m_cv1_bn_weight] + [
                                        10 * weight_alpha_list2[t] * block_t[t].cv2.bn.weight.data.abs().clone() for t
                                        in range(len_bottleneck)]
                ...
                assert torch.sum(mask_list2[0]) == torch.sum(mask_list2[1]) and torch.sum(mask_list2[0]) == torch.sum(
                    mask_list2[-1])
                c_ = torch.sum(mask_list2[0]).cpu().numpy()

                mask_list.append([mask_list1, mask_list2, mask_cv2, mask_cv3])
                o_ch_list.append([[mask_list1[tt].shape[0] for tt in range(len(mask_list1))],
                                  [mask_list2[tt].shape[0] for tt in range(len(mask_list2))], mask_cv2.shape[0],
                                  mask_cv3.shape[0]])
                n_ch_list.append([[int(torch.sum(mask_list1[tt])) for tt in range(len(mask_list1))],
                                  [int(torch.sum(mask_list2[tt])) for tt in range(len(mask_list2))],
                                  int(torch.sum(mask_cv2)), int(torch.sum(mask_cv3))])
                e_list.append(
                    [torch.sum(mask_list2[0]).cpu().numpy() / c_out, torch.sum(mask_cv2).cpu().numpy() / c_out])
                ein_list.append([torch.sum(mask_list1[tt]).cpu().numpy() / c_ for tt in range(len_bottleneck)])
            elif isinstance(m, SPP):
                mask_cv1 = cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=m.cv1.conv,
                                                      block_bn=m.cv1.bn)
                mask_cv2 = cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=m.cv2.conv,
                                                      block_bn=m.cv2.bn)
            ...
            if isinstance(m, nn.Upsample):
                ...
                len_bottleneck = len(block_t)
                if i < len_backbone:
                    assert backbone_ifo[i][3][1] == False and isinstance(block_t,
                                                                         nn.Sequential) and len_bottleneck == int(
                        backbone_ifo[i][1])
                else:
                    assert head_ifo[i - len_backbone][3][1] == False and isinstance(block_t,
                                                                                    nn.Sequential) and len_bottleneck == int(
                        head_ifo[i - len_backbone][1])

                mask_list1 = [cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=block_t[tt].cv1.conv,
                                                         block_bn=block_t[tt].cv1.bn) for tt in range(len_bottleneck)]
                mask_list2 = [cal_mask_weightingByKernel(thresh, opt.perlayer_ratio, block_conv=block_t[tt].cv2.conv,
                                                         block_bn=block_t[tt].cv2.bn) for tt in range(len_bottleneck)]
                ...
                for tt in range(len_bottleneck):
                    ein_list_temp += [torch.sum(mask_list2[tt]), torch.sum(mask_list1[tt]),
                                      torch.sum(mask_list2[tt + 1])]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='runs/train/exp76/weights/last_190.pt',
                        help='initial weights path')
    parser.add_argument('--device', default='3', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--batch-size', type=int, default=8, help="Total batch size for all gpus.")
    parser.add_argument("--overall_ratio", default=0.2, type=float, help='pruning rate')
    parser.add_argument("--perlayer_ratio", default=0.1, type=float, help='pruning protection rate')
    parser.add_argument("--save", default='/home/shw/code/yolov5-new/yolov5/runs/train/exp76', type=str,
